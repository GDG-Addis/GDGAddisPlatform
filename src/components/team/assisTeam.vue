  ...
  <v-container class="pa-0 mb-5">
    <v-layout wrap align-start justify-start row fill-height class="mt-0 mb-0">
      <v-flex xs12 class="mb-3">
        <p class="google-font" style="font-size:130%;">Assistant Team Members</p>
        <v-divider></v-divider>
      </v-flex>
        ...
        <v-list three-line class="pa-2">
          <template>
            <v-list-tile :key="item.name" avatar>
              <v-avatar size="80">
                <v-img :src="getImgUrl(item.profileImage)" :lazy-src="getImgUrl(item.profileImage)" class="mr-3">
...
<script>
import TeamDetails from "@/assets/data/assisTeam.json";
export default {
  data() {
    return {
